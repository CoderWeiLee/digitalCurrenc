...
- (void)awakeFromNib {
    ...
    self.backView.backgroundColor = blackcolor;
}

- (void)setModel:(symbolModel *)model{
    _model = model;
    self.symbolLabel.text = model.symbol;
    self.symbolLabel.textColor = [UIColor whiteColor];//AppTextColor_Level_1;
    
    self.priceLabel.text = [NSString stringWithFormat:@"%.2f",[model.close doubleValue]];
    self.priceLabel.textColor = AppTextColor_Level_2;
