...
@interface HomeViewController ()<SocketDelegate,chatSocketDelegate,xindelegate>
{
...
- (void)gesturePassword{
    if ([ZLGestureLockViewController gesturesPassword].length > 0) {
        ...
        if (![[NSUserDefaults standardUserDefaults] boolForKey:kShowGesturePassword]) {
            ...
            .LeeAddAction(^(LEEAction *action) {
                action.title = LocalizationKey(@"ok");
                action.titleColor = AppTextColor_Level_1;
                action.font = [UIFont systemFontOfSize:16];
                action.clickBlock = ^{
...
- (void)leftItem{
    ...
    imageView.image = [UIImage imageNamed:@"hone_new"];
    //添加到导航条
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    [self.navigationController.navigationBar setBackgroundColor:blackcolor];
  //  self.navigationController.navigationBar.clipsToBounds = YES;
    [[UINavigationBar appearance] setBackgroundImage:[[UIImage alloc] init] forBarMetrics:UIBarMetricsDefault];
    [[UINavigationBar appearance] setShadowImage:[[UIImage alloc] init]];
...
-(void)setTablewViewHeard{
    ...
    hedaview.backgroundColor = blackcolor;
    self.bannerView = [[JKBannarView alloc]initWithFrame:CGRectMake(0, 0, kWindowW, 150 * kWindowWHOne) viewSize:CGSizeMake(kWindowW,150 * kWindowWHOne)];
...
-(void)destory{
//    DCDistroyDealVC * vc = [[DCDistroyDealVC alloc] init];
//    [[AppDelegate sharedAppDelegate] pushViewController: vc];
    wwwwwViewController *vc = [[wwwwwViewController alloc]init];
    [[AppDelegate sharedAppDelegate] pushViewController:vc];
}
...
//    wwwwwViewController *vc = [[wwwwwViewController alloc]init];
//    [[AppDelegate sharedAppDelegate] pushViewController:vc];
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:nil message:@"只针对abc10万以上用户" preferredStyle:UIAlertControllerStyleAlert];
    [alert addAction:[UIAlertAction actionWithTitle:[[ChangeLanguage bundle] localizedStringForKey:@"ok" value:nil table:@"English"] style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
    }]];
    [self presentViewController: alert animated: YES completion: nil];
}
-(void)Bdeal{
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:nil message:@"只针对abc内部用户" preferredStyle:UIAlertControllerStyleAlert];
    [alert addAction:[UIAlertAction actionWithTitle:[[ChangeLanguage bundle] localizedStringForKey:@"ok" value:nil table:@"English"] style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
       // cancelBlock();
    }]];
    [self presentViewController: alert animated: YES completion: nil];
...
//            return sectionView;
        ...
        if(@available(iOS 13.0, *)) {
            headview.backgroundColor = [UIColor systemGroupedBackgroundColor];
        }
        else {
