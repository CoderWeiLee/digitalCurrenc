...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(xinlogin) name:@"xinlogin" object:nil];
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"addminecell" bundle:nil] forCellReuseIdentifier:@"addminecell"];
...
- (void)getUserMa{
    ...
    [MineNetManager getSetMyinf:nil CompleteHandle:^(id resPonseObj, int code) {
        
        _inviter = resPonseObj[@"inviter"];
...
-(NSArray *)getNameArr{
    ...
    NSArray * nameArr = @[LocalizationKey(@"myBill"),
                          [[ChangeLanguage bundle] localizedStringForKey:@"myAdvertising" value:nil table:@"English"],
                          [[ChangeLanguage bundle] localizedStringForKey:@"myEntrust" value:nil table:@"English"],
                          //LocalizationKey(@"myPromotion"),
                          [[ChangeLanguage bundle] localizedStringForKey:@"safeCenter" value:nil table:@"English"],
                          [[ChangeLanguage bundle] localizedStringForKey:@"settingCenter" value:nil table:@"English"],
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        ...
        addminecell *cell = [tableView dequeueReusableCellWithIdentifier:@"addminecell" forIndexPath:indexPath];
        if(![YLUserInfo isLogIn]){
            cell.label.text = @"L0";
        }else{
            if ([NSString stringWithFormat:@"%@",[YLUserInfo shareUserInfo].levelName].length == 0) {
                cell.label.text = @"L0";
            }else{
          cell.label.text = [NSString stringWithFormat:@"L%@",[YLUserInfo shareUserInfo].levelName];
            }
        }
        
        if (_inviter.length) {
            cell.bindMa.hidden = YES;
        }
        ...
        cell.selectBlock = ^(NSInteger num) {
            if(![YLUserInfo isLogIn]){
                ...
                switch (num) {
                    case 0:{
                        //我的推广
                        wodetuiguangViewController *billVC = [[wodetuiguangViewController alloc] init];
                    ...
                    case 1000:{
                        ...
                        UIAlertController *alert = [UIAlertController alertControllerWithTitle:LocalizationKey(@"inputinvite") message:nil preferredStyle:UIAlertControllerStyleAlert];
                            [alert addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
                                textField.placeholder = LocalizationKey(@"inputinvite");
                            ...
                            [alert addAction:[UIAlertAction actionWithTitle:LocalizationKey(@"ok") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                                ...
                                if (!envirnmentNameTextField.text.length) {
                                    [self.view makeToast:@"请先填写验证码" duration:1.5 position:CSToastPositionCenter];

                                    return;
                                }
                                
                                NSMutableDictionary*dict = [NSMutableDictionary dictionary];
                                dict[@"promotion"]= envirnmentNameTextField.text;
                                
                                
                                [MineNetManager getSetpromotion:dict CompleteHandle:^(id resPonseObj, int code) {
                                    
                                    if([resPonseObj[@"code"] integerValue] == 0){
                                        
                                        [self.view makeToast:@"绑定成功" duration:1.5 position:CSToastPositionCenter];
