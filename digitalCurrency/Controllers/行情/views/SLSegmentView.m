...
- (void)layoutSubviews
{
    ...
    self.bottomLine.frame = CGRectMake(0, self.height-4, self.width, 4);
    //每个子segment的宽度
    ...
    self.canSliderLine.height = 4.0f;
}
...
- (void)createSegments
{
    ...
    for (int i=0; i<self.segementTitleArray.count; i++) {
        ...
        [button setTitleColor: [UIColor colorWithRed:20/255.0 green:108/255.0 blue:233/255.0 alpha:1.0] forState:UIControlStateSelected];
...
-(UIView *)canSliderLine
{
    if (!_canSliderLine) {
        ...
        _canSliderLine.backgroundColor =  RGBOF(0x1995FE);
    }
