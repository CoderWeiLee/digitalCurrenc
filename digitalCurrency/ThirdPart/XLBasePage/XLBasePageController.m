...
- (void)viewDidLoad {
    ...
    self.titleFont = self.titleFont?self.titleFont:[UIFont fontWithName:@"Helvetica-Bold"size:14];
    ...
    if ([self.dataSource respondsToSelector:@selector(numberViewControllersInViewPager:)]) {
        ...
        for (int i = 0; i<totalVC; i++) {
            ...
            if ([self.dataSource respondsToSelector:@selector(viewPager:titleWithIndexViewControllers:)]) {
                ...
                button.frame = CGRectMake(XXXX,5,60,30);
                XXXX += 60;
        ...
        UILabel *line = [[UILabel alloc]initWithFrame:CGRectMake(0,36, kWindowW, 1)];
...
-(void)drawRect:(CGRect)rect
{
    if (self.selected) {
        ...
        CGContextMoveToPoint(ctx, 12, self.frame.size.height-lineWidth + 2);
        CGContextAddLineToPoint(ctx, self.frame.size.width - 12, self.frame.size.height-lineWidth + 2);
